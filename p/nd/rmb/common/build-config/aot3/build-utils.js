...
function getEnvFile(suffix) {
  ...
  let fileName = helpers.root(`client/src/lib/aot3/environments/environment${suffix}.ts`);
  if (fs.existsSync(fileName)) {
    return fileName;
  } else if (fs.existsSync(fileName = helpers.root('client/src/lib/aot3/environments/environment.ts'))) {
...
function ngcWebpackSetup(prod, metadata) {
  ...
  if (environment) {
    ngcWebpackPluginOptions.hostReplacementPaths = {
      [helpers.root('client/src/lib/aot3/environments/environment.ts')]: environment
    }
  }
